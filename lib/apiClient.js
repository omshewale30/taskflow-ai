...
export const apiClient = {
  ...
  getDailyDigest: async () => {
    try {
      ...
      console.error('API Error (getDailyDigest):', error.response?.data || error.message);
      throw error;
    }
  },

  updateTaskImportance: async (taskId, isImportant) => {
    try {
      const response = await api.put(`/tasks/${taskId}/importance`, { is_important: isImportant });
      
      return response.data;
    } catch (error) {
      console.error('API Error (updateTaskImportance):', error.response?.data || error.message);
      throw error;
    }
  },
  ...
  deleteTask: async (taskId) => {
    try {
      ...
      console.error('API Error (deleteTask):', error.response?.data || error.message);
      throw error;
    }
  }
};